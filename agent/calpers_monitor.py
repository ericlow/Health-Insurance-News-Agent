...
LISTING_URL = 'https://www.calpers.ca.gov/newsroom/calpers-news'
BASE_URL = 'https://www.calpers.ca.gov'
...
def run_monitor() -> tuple[int, list[int]]:
    ...
    try:
        ...
        print(f'[calpers-monitor] {len(entries)} found, {len(new_ids)} new.')
        return run_id, new_ids
    except Exception as exc:
        _fail_run(conn, run_id, str(exc))
        print(f'[calpers-monitor] failed: {exc}')
        return run_id, []
    finally:
...
def _fetch_listing() -> list[dict]:
    ...
    for li in soup.find_all('li'):
        a = li.find('a', href=lambda h: h and '/newsroom/calpers-news/' in h)
        if not a:
            continue
        url = a['href']
        ...
        if not title:
            continue
        # Date is the last text segment in the <li> after the link; format: "Month DD, YYYY"
