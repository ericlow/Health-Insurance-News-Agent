...
LISTING_URL = 'https://www.providence.org/business/press-room/press-releases/california'
...
def run_monitor() -> tuple[int, list[int]]:
    ...
    try:
        ...
        print(f'[providence-monitor] {len(entries)} found, {len(new_ids)} new.')
        return run_id, new_ids
    except Exception as exc:
        _fail_run(conn, run_id, str(exc))
        print(f'[providence-monitor] failed: {exc}')
        return run_id, []
    finally:
...
def _fetch_listing() -> list[dict]:
    ...
    seen = set()
    entries = []
    for a in soup.select('a[href*="/news/uf/"]'):
        path = a['href'].split('?')[0]
        ...
        title = _DATE_RE.sub('', raw_text).strip() if date_match else raw_text
        if not title:
            continue
        entries.append({
            'url': url,
            'title': title,
            'published_at': _parse_date(date_match.group(1) if date_match else None),
            'body_text': None,
            'category': None,
            'tags': [],
        })
