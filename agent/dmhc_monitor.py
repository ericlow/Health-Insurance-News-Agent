...
SOURCE = 'dmhc.ca.gov'
LISTING_URL = 'https://www.dmhc.ca.gov/Resources/Newsroom/PressReleases.aspx'
BASE_URL = 'https://www.dmhc.ca.gov'

# Akamai CDN blocks standard requests; curl_cffi with Chrome impersonation is required.
...
def run_monitor() -> tuple[int, list[int]]:
    ...
    try:
        ...
        print(f'[dmhc-monitor] {len(entries)} found, {len(new_ids)} new.')
        return run_id, new_ids
    except Exception as exc:
        _fail_run(conn, run_id, str(exc))
        print(f'[dmhc-monitor] failed: {exc}')
        return run_id, []
    finally:
...
def _fetch_listing() -> list[dict]:
    resp = cffi_requests.get(LISTING_URL, impersonate=_IMPERSONATE, timeout=30)
    ...
    for p in soup.find_all('p'):
        ...
        link = p.find('a', href=lambda h: h and 'PressReleases/' in h)
