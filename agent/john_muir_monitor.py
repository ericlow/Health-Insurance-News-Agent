...
LISTING_URL = 'https://www.johnmuirhealth.com/about-john-muir-health/press-room.html'
BASE_URL = 'https://www.johnmuirhealth.com'
...
def run_monitor() -> tuple[int, list[int]]:
    ...
    try:
        ...
        print(f'[john-muir-monitor] {len(entries)} found, {len(new_ids)} new.')
        return run_id, new_ids
    except Exception as exc:
        _fail_run(conn, run_id, str(exc))
        print(f'[john-muir-monitor] failed: {exc}')
        return run_id, []
    finally:
