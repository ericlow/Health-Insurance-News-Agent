...
FEED_URL = 'https://kffhealthnews.org/state/california/feed/'
HEADERS = {'User-Agent': 'Mozilla/5.0 (compatible; HealthInsuranceNewsAgent/1.0)'}


def run_monitor() -> tuple[int, list[int]]:
    """Regular monitor run — fetches the KFF California RSS feed.
    ...
    try:
        ...
        print(f'[kff-monitor] {len(entries)} found, {len(new_ids)} new.')
        return run_id, new_ids
    except Exception as exc:
        _fail_run(conn, run_id, str(exc))
        print(f'[kff-monitor] failed: {exc}')
        return run_id, []
    finally:
...
def _fetch_feed() -> list[dict]:
    ...
    for e in feed.entries:
        ...
        entries.append({
            ...
            'category': _extract_category_from_url(e.link),
