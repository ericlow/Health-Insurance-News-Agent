...
FEED_URL = 'https://www.healthcaredive.com/feeds/topic/payers/'
MAX_ENTRIES = 20


def run_monitor() -> tuple[int, list[int]]:
    """Regular monitor run — fetches the Healthcare Dive Payers RSS feed.
    ...
    try:
        ...
        print(f'[healthcare-dive-monitor] {len(entries)} found, {len(new_ids)} new.')
        return run_id, new_ids
    except Exception as exc:
        _fail_run(conn, run_id, str(exc))
        print(f'[healthcare-dive-monitor] failed: {exc}')
        return run_id, []
    finally:
...
def _fetch_feed() -> list[dict]:
    ...
    for e in feed.entries[:MAX_ENTRIES]:
        # Skip sponsored content (/spons/ URLs)
        if '/spons/' in e.link:
            continue
        entries.append({
            'url': e.link,
...
def _fetch_article_body(url: str) -> str | None:
    """Fetch article body text from the Healthcare Dive article page."""
    try:
