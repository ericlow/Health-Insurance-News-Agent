...
def _run_with_mocks(
    ...
    with patch('scheduler.run_scrape', return_value=beckers_return) as m_scrape, \
        ...
        run_pipeline()
        return m_scrape, m_kff, m_cigna, m_triage, m_summ, m_discord, m_health_check
...
def test_all_stages_called():
    m_scrape, m_kff, m_cigna, m_triage, m_summ, m_discord, m_health_check = _run_with_mocks()
    ...
    assert m_health_check.call_count == 3  # once per source
...
def test_health_check_called_for_beckers_with_correct_count():
    _, _, _, _, _, _, m_hc = _run_with_mocks(beckers_return=(10, [1, 2, 3]))
...
def test_health_check_called_for_kff_with_correct_count():
    _, _, _, _, _, _, m_hc = _run_with_mocks(kff_return=(20, [7, 8]))
    calls = m_hc.call_args_list
...
def test_health_check_called_for_cigna_with_correct_count():
    _, _, _, _, _, _, m_hc = _run_with_mocks(cigna_return=(30, [9, 10, 11]))
    calls = m_hc.call_args_list
...
def test_health_check_called_with_zero_when_no_new_articles():
    _, _, _, _, _, _, m_hc = _run_with_mocks(
        beckers_return=(1, []),
        kff_return=(2, []),
        cigna_return=(3, []),
    )
