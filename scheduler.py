...
from agent.covered_ca_monitor import run_monitor as covered_ca_run_monitor, FEED_URL as COVERED_CA_FEED_URL
...
def run_pipeline():
    ...
    _covered_ca_run_id, covered_ca_new_ids = covered_ca_run_monitor()
    ...
    log.info(
        ...
        len(hoag_new_ids), len(john_muir_new_ids), len(covered_ca_new_ids),
    )
    ...
    post_health_check("Covered California", fetch_verdicts_for_articles(covered_ca_new_ids),
                      web_url='https://www.coveredca.com/newsroom/news-releases/', feed_url=COVERED_CA_FEED_URL)
