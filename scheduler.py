...
from agent.dmhc_monitor import run_monitor as dmhc_run_monitor, LISTING_URL as DMHC_LISTING_URL
...
def run_pipeline():
    ...
    _dmhc_run_id, dmhc_new_ids = dmhc_run_monitor()
    ...
    log.info(
        ...
        len(hoag_new_ids), len(john_muir_new_ids), len(dmhc_new_ids),
    )
    ...
    post_health_check("DMHC", fetch_verdicts_for_articles(dmhc_new_ids),
                      web_url=DMHC_LISTING_URL, feed_url=DMHC_LISTING_URL)
