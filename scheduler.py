...
from agent.healthcare_dive_monitor import run_monitor as healthcare_dive_run_monitor, FEED_URL as HEALTHCARE_DIVE_FEED_URL
...
def run_pipeline():
    ...
    _healthcare_dive_run_id, healthcare_dive_new_ids = healthcare_dive_run_monitor()
    ...
    combined_ids = (beckers_new_ids + kff_new_ids + cigna_new_ids + sutter_new_ids + uc_davis_new_ids
                    ...
                    + healthcare_dive_new_ids)
    log.info(
        "[scheduler] %d new articles (%d Becker's, %d KFF, %d Cigna, %d Sutter, %d UC Davis, %d UCSD,"
        " %d UCI Health, %d UCLA Health, %d UCSF, %d Sharp, %d Scripps, %d Providence, %d Hoag,"
        " %d John Muir, %d Healthcare Dive).",
        ...
        len(hoag_new_ids), len(john_muir_new_ids), len(healthcare_dive_new_ids),
    )
    ...
    post_health_check("Hoag Health", fetch_verdicts_for_articles(hoag_new_ids),
    ...
    post_health_check("Healthcare Dive (Payers)", fetch_verdicts_for_articles(healthcare_dive_new_ids),
                      web_url='https://www.healthcaredive.com/topic/payers/', feed_url=HEALTHCARE_DIVE_FEED_URL)
