...
def run_pipeline():
    ...
    combined_ids = beckers_new_ids + kff_new_ids + cigna_new_ids + sutter_new_ids + uc_davis_new_ids + ucsd_new_ids + uci_health_new_ids + ucla_health_new_ids + ucsf_new_ids
    log.info(
        "[scheduler] %d new articles (%d Becker's, %d KFF, %d Cigna, %d Sutter, %d UC Davis, %d UCSD, %d UCI Health, %d UCLA Health, %d UCSF).",
        len(combined_ids), len(beckers_new_ids), len(kff_new_ids), len(cigna_new_ids), len(sutter_new_ids),
        len(uc_davis_new_ids), len(ucsd_new_ids), len(uci_health_new_ids), len(ucla_health_new_ids), len(ucsf_new_ids),
    )
