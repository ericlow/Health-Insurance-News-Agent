...
from agent.scan_monitor import run_monitor as scan_run_monitor, LISTING_URL as SCAN_LISTING_URL
...
def run_pipeline():
    ...
    _scan_run_id, scan_new_ids = scan_run_monitor()
    ...
    log.info(
        ...
        len(hoag_new_ids), len(john_muir_new_ids), len(scan_new_ids),
    )
    ...
    post_health_check("SCAN Health Plan", fetch_verdicts_for_articles(scan_new_ids),
                      web_url=SCAN_LISTING_URL, feed_url=SCAN_LISTING_URL)
