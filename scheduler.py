...
from agent.pih_health_monitor import run_monitor as pih_health_run_monitor, LISTING_URL as PIH_HEALTH_LISTING_URL
...
def run_pipeline():
    ...
    _pih_health_run_id, pih_health_new_ids = pih_health_run_monitor()
    ...
    log.info(
        ...
        len(hoag_new_ids), len(john_muir_new_ids), len(pih_health_new_ids),
    )
    ...
    post_health_check("PIH Health", fetch_verdicts_for_articles(pih_health_new_ids),
                      web_url=PIH_HEALTH_LISTING_URL, feed_url=PIH_HEALTH_LISTING_URL)
